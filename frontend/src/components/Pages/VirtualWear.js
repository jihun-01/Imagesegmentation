...
const getApiUrl = () => {
  ...
  const processVirtualTryOn = async (handImageFile) => {
    ...
    try {
      ...
      console.log('가상 착용 요청 시작...');
      
      const response = await fetch(`${getApiUrl()}/virtual-try-on`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (responseData.success) {
        ...
        console.log('가상 착용 완료:', responseData.message);
  ...
  return (
    <div className="max-w-screen-sm mx-auto flex flex-col min-h-[calc(100vh-56px)] bg-gray-100 px-4 py-8">
      <div className="w-full h-full bg-white rounded-2xl shadow-lg p-10 flex flex-col items-center">
        ...
        {/* 안내 메시지 */}
        <p className="text-black text-lg font-semibold mb-8 mt-2 text-center">
